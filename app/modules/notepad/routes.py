...
@login_required
def get_notepad(notepad_id: int):
    notepad = notepad_service.get_or_404(notepad_id)
    if notepad.user_id != current_user.id:
        flash('You are not authorized to view this notepad!', 'danger')
...
@login_required
def edit_notepad(notepad_id: int):
    notepad = notepad_service.get_or_404(notepad_id)
    if notepad.user_id != current_user.id:
...
@login_required
def delete_notepad(notepad_id: int):
    notepad = notepad_service.get_or_404(notepad_id)
    if notepad.user_id != current_user.id:
